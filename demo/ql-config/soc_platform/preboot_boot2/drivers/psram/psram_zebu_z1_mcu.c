...
/* Unused macros reserved for alignment with ASIC code */
...
#ifndef PSRAM_DVC_X8
    REG32(PSRAM_BASE + 0x0004) = 0x3009005D;    //32M byte
#else
...
//reseved and if 0 crw_mcu_psc_wakeup for alignment with ASIC code
#if 0
// type=2 mcu psc wakeup
void crw_mcu_psc_wakeup(void)
{
    unsigned read_data;
    unsigned type = 2;
    unsigned mcu_psc;
