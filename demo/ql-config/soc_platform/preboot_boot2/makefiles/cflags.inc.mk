...
# cc-option
# Usage: CFLAGS += $(call cc-option,-mthumb)
cc-option = $(call __cc-option, $(CC),$(CFLAGS),$(1),$(2))

# ld-option
# Usage: LDFLAGS += $(call ld-option, -X, -Y)
ld-option = $(call try-run, $(LD) $(LDFLAGS) $(1) -v,$(1),$(2),$(3))

LDARG += $(call ld-option,--no-warn-rwx-segment)

LDARGPREFIX = -Wl,
LDARG := $(strip $(LDARG))
ifneq (,$(LDARG))
  LINKFLAGS += $(addprefix $(LDARGPREFIX), $(LDARG))
endif
